...
export async function aggiungiCarta(
  ...
  const idProductRaw = String(formData.get("id_product") ?? "").trim();
  const idProduct = idProductRaw ? Number(idProductRaw) : null;
  ...
  if (idProductRaw && (!Number.isFinite(idProduct) || (idProduct as number) <= 0)) {
    return { error: "L'ID Cardmarket deve essere un numero positivo." };
  }
  ...
  // Upsert in fusion_world_cards: match su id_product se presente, altrimenti
  // nuova riga sempre con source = 'manuale' (nessun import bulk catalogo).
  let cardId: number;
  if (idProduct !== null) {
    ...
    const { data, error } = await admin
      .from("fusion_world_cards")
      .insert({ name, set_code: setCode, card_number: cardNumber, product_type: productType, source: "manuale" })
      .select("id")
      .single();
    if (error) return { error: error.message };
    cardId = data.id as number;
