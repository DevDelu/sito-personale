...
export async function Hobbies() {
  ...
  return (
    <section id="passioni" className="mx-auto w-full max-w-5xl border-t border-border px-6 py-16">
      <RevealGroup>
        <RevealItem>
          <SectionEyebrow>passioni</SectionEyebrow>
          <h2 className="font-sans text-2xl font-bold tracking-tight">{t("title")}</h2>
        </RevealItem>
        <RevealItem className="mt-8 grid grid-cols-1 gap-px overflow-hidden rounded-md border border-border bg-border sm:grid-cols-2 lg:grid-cols-4">
          {items.map((item, index) => {
            const Icon = HOBBY_ICONS[index % HOBBY_ICONS.length];
            return (
              <div key={item.title} className="flex flex-col gap-2 bg-surface p-5">
